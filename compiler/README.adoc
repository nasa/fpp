...
To compile a JVM program to a native binary, you must
first run the program with a tracing agent enabled and record the results,
so that `native-image` can see the results when it runs.
The directory `lib/src/main/resources/META-INF` stores the results of
running the unit tests with the tracing agent enabled.
