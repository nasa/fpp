...
To compile a JVM program to a native binary, GraalVM requires that the
JVM program first be run with a tracing agent enabled, so the results
of the run can be recorded and communicated to `native-image`.
The tracing agent has already been run for all the FPP tools on all the
unit test inputs, and the results are stored in `lib/src/main/resources/META-INF`.
...
Remember to re-run `./install` to install the standard version
