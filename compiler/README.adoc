...
IMPORTANT: Make sure to have native-image installed before running trace.
...
. Set variable `FPRIME` to an F Prime reposirory to be traced.

. Run `trace-fprime` to trace fprime projects that run in CI.
