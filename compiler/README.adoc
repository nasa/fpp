...
Note that `install-trace` uses the native image files to run unit
tests, which may fail at this point.
...
. If you wish, run `./install-trace` again to build the native
image files with the new trace files and run the unit tests.
All the tests should pass at this point.
Alternatively you can let CI on `nasa/fpp` do this.
