...
module template T(interface i1: A_I1, interface i2: A_I2, constant idx: U32) {
    constant a = 0
    constant b = a + 1
    ...
    topology Top {
        ...
        # Import `a2` using `A_I2` interface
        instance i2
        ...
        connections C {
            ...
            i1.bOut[idx] -> b.bIn[idx]

            # Connect to a port inside `A_I2` on the wrong alias
            b.aOut[idx] -> i1.aIn2[idx]
        }
    }
}
...
module M1 {
    expand T(interface a1, interface a1, constant 0)
}
