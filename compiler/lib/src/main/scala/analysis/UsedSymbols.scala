  ...
  def resolveUses(a: Analysis, ss: Set[Symbol]): Set[Symbol] = {
    ...
    def resolveNode(s: Symbol): Set[Symbol] = {
      val Right(a2) = s match {
        ...
        case Symbol.TemplateConstantParam(_, expr) => exprNode(a, expr)
        case Symbol.TemplateTypeParam(_, tn) => typeNameNode(a, tn)
        case Symbol.TemplateInterfaceParam(_, name) => interfaceInstanceUse(a, name, Name.Qualified.fromQualIdent(name.data))
      }
