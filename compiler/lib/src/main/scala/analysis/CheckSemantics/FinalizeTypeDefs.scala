  ...
  extends TypeExpressionAnalyzer
{
  ...
  override def defAliasTypeAnnotatedNode(a: Analysis, aNode: Ast.Annotated[AstNode[Ast.DefAliasType]]) = {
    ...
    def visitor(a: Analysis, aNode: Ast.Annotated[AstNode[Ast.DefAliasType]]) = {
      ...
      val aliasType @ Type.AliasType(_, _) = a.typeMap(node.id).runtimeChecked
      for {
  ...
  override def defArrayAnnotatedNode(a: Analysis, aNode: Ast.Annotated[AstNode[Ast.DefArray]]) = {
    ...
    def visitor(a: Analysis, aNode: Ast.Annotated[AstNode[Ast.DefArray]]) = {
      ...
      val arrayType @ Type.Array(_, _, _, _) = a.typeMap(node.id).runtimeChecked
      for {
        ...
        default <- data.default match {
          case Some(defaultNode) => {
            ...
            for (_ <- Analysis.convertTypes(loc, v.getType -> arrayType))
              yield {
                val array @ Value.Array(_, _) = Analysis.convertValueToType(v, arrayType).runtimeChecked
                array
              }
          }
          case None => {
            val Some(anonArray) = arrayType.anonArray.getDefaultValue.runtimeChecked
  ...
  override def defEnumAnnotatedNode(a: Analysis, aNode: Ast.Annotated[AstNode[Ast.DefEnum]]) = {
    ...
    def visitor(a: Analysis, aNode: Ast.Annotated[AstNode[Ast.DefEnum]]) = {
      val (_, node, _) = aNode
      val data = node.data
      val enumType @ Type.Enum(_, _, _) = a.typeMap(node.id).runtimeChecked
      val default = data.default match {
        case Some(default) => a.valueMap(default.id)
        case None => a.valueMap(data.constants.head._2.id)
      }
      val enumConstant @ Value.EnumConstant(_, _) = default.runtimeChecked
  ...
  override def defStructAnnotatedNode(a: Analysis, aNode: Ast.Annotated[AstNode[Ast.DefStruct]]) = {
    ...
    def visitor(a: Analysis, aNode: Ast.Annotated[AstNode[Ast.DefStruct]]) = {
      ...
      val structType @ Type.Struct(_, _, _, _, _) = a.typeMap(node.id).runtimeChecked
      for {
        ...
        structType <- {
          val anonStructType @ Type.AnonStruct(_) = t.runtimeChecked
        ...
        default <- data.default match {
          case Some(defaultNode) => {
            ...
            for (_ <- Analysis.convertTypes(loc, v.getType -> structType))
              yield {
                val struct @ Value.Struct(_, _) = Analysis.convertValueToType(v, structType).runtimeChecked
                struct
              }
          }
          case None => {
            val Some(anonStruct) = structType.anonStruct.getDefaultValue.runtimeChecked
