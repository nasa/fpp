...
implicit val symbolMapEncoder: Encoder[Map[Symbol, Symbol]] = Encoder.instance { symbols =>
  ...
  //approved but maybe consider creating a specifier encoder that is more concise
    ...
    //approved? (consider switching positions of key 1 and key 2)
    implicit val locationSpecifierMapEncoder: Encoder[Map[(Ast.SpecLoc.Kind, Name.Qualified), Ast.SpecLoc]] = Encoder.instance { symbols =>
      symbols.toList.map {
        case ((key1, key2), value) => getUnqualifiedClassName(key1) -> Json.obj(
    ...
    implicit val componentInstanceLocationMapEncoder: Encoder[Map[ComponentInstance, (Ast.Visibility, Location)]] = Encoder.instance { symbols =>
      symbols.toList.map {
        case (key, value) => key.aNode._2.id -> value.asJson
      }.toMap.asJson
    }
    ...
    implicit val nameGroupSymbolMapEncoder: Encoder[Map[NameGroup, NameSymbolMap]] = Encoder.instance { symbols =>
      symbols.toList.map {
        case (key, value) => getUnqualifiedClassName(key) -> value.asJson 
      }.toMap.asJson
    }


    //approved?
    implicit val nameSymbolMapEncoder: Encoder[Map[Name.Unqualified, Symbol]] = Encoder.instance { symbols =>
      symbols.toList.map {
        case (key, value) => key.toString -> value.getNodeId
      }.toMap.asJson
    }


    //approved
    implicit val directImportMapEncoder: Encoder[Map[Symbol.Topology, Location]] = Encoder.instance { symbols =>
      symbols.toList.map {
        case (key, value) => key.getNodeId -> value.asJson
      }.toMap.asJson
    }
    ...
    //approved
    implicit val useDefMapEncoder: Encoder[Map[AstNode.Id, Symbol]] = Encoder.instance { symbols =>
      symbols.toList.map {
        case (key, value) => key -> value.getUnqualifiedName
      }.toMap.asJson
    }

    //key.toString must be changed
    implicit val ConnectionMapEncoder: Encoder[Map[PortInstanceIdentifier, Set[Connection]]] = Encoder.instance { symbols =>
      symbols.toList.map {
        case (key, value) => key.toString -> value.asJson
      }.toMap.asJson
    }
     ...
     //key.toString must be changed
    implicit val PortNumberMapEncoder: Encoder[Map[Connection, Int]] = Encoder.instance { symbols =>
      symbols.toList.map {
        case (key, value) => key.toString -> value.asJson
      }.toMap.asJson
    }
    ...
    //approved
    implicit val patternMapEncoder: Encoder[Map[Ast.SpecConnectionGraph.Pattern.Kind, ConnectionPattern]] = Encoder.instance { symbols =>
      symbols.toList.map {
        case (key, value) => getUnqualifiedClassName(key) -> value.asJson
      }.toMap.asJson
    }
    ...
    implicit val tlmChannelEncoder: Encoder[TlmChannel] = Encoder.instance { tlmChannel =>
      Json.obj(
    ...
    implicit val typeEncoder: Encoder[Type] = Encoder.encodeString.contramap(_.toString())
    implicit val valueEncoder: Encoder[Value] = Encoder.encodeString.contramap(_.toString)

    implicit val limitsEncoder: Encoder[TlmChannel.Limits] = Encoder.instance { limits =>
      limits.map {
        case (kind, (id, value)) => kind.toString -> Json.obj(
          "id" -> id.asJson,
    ...
    implicit val portInstanceIdentifierEncoder: Encoder[PortInstanceIdentifier] = Encoder.encodeString.contramap(_.toString())

    implicit val componentEncoder: Encoder[Component] = Encoder.instance { component =>
      Json.obj(
    ...
    implicit val generalPortInstanceKindEncoder: Encoder[PortInstance.General.Kind] = Encoder.encodeString.contramap(getUnqualifiedClassName(_))

    implicit val generalPortInstancetEncoder: Encoder[PortInstance.General] = Encoder.instance { genPort =>
      Json.obj(
    ...
    implicit val specialPortInstanceEncoder: Encoder[PortInstance.Special] = Encoder.instance { specPort =>
      Json.obj(
    ...
    implicit val internallPortInstancetEncoder: Encoder[PortInstance.Internal] = Encoder.instance { intPort =>
      Json.obj(
    ...
    implicit val nonParamCommandEncoder: Encoder[Command.NonParam] = Encoder.instance { command =>
      Json.obj(
    ...
    implicit val paramCommandEncoder: Encoder[Command.Param] = Encoder.instance { param =>
      Json.obj(
        "id" -> param.aNode._2.id.asJson,
    ...
    implicit val componentInstanceEncoder: Encoder[ComponentInstance] = Encoder.instance {compInstance =>
      Json.obj(
    ...
    implicit val InitSpecifierEncoder: Encoder[InitSpecifier] = Encoder.instance {initSpec =>
    Json.obj(
    ...
    implicit val topologyEncoder: Encoder[Topology] = Encoder.instance {topology =>
      Json.obj(
        "id" -> topology.aNode._2.id.asJson,
        "directImportMap" -> topology.directImportMap.asJson,
        "transitiveImportSet" ->  Json.arr(topology.transitiveImportSet.map(_.getNodeId).map(_.asJson).toSeq: _*),
    ...
    implicit val connectionPatternEncoder: Encoder[ConnectionPattern] = Encoder.instance {connectionPattern =>
      Json.obj(
