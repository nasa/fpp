...
[source,fpp]
----
struct s { x : U32 = 0 } U32 # Defines s in the type name group
...
qualified name _I_ if it exists in name group _S_. Otherwise an error
results.
...
... Determine the name group _S_ of _Q_ `.` _I_.
