...
module M {
  enum E { X = 0, Y = 1 }
  array A = [3] E # E is an enum type name. It names the type M.E.
}
----
...
module M {
  ...
  struct T {
    s : S # S is a struct type name. It names the type M.S.
  }
}
----
...
module M {
  type T
  array A = [3] T # T is an abstract type name. It names the type M.T.
}
----
