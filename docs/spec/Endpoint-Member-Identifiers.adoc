== Endpoint Instance Member Identifiers

A *endpoint instance member identifier* refers to a member of an
endpoint instance. Endpoint instances refer to a definition that
contains **port instances** that can be connected to in a
<<Specifiers_Connection-Graph-Specifiers,connection graph>>.

The two types of endpoint instances are <<Definitions_Topology-Definitions,topologies>>
and <<Definitions_Component-Instance-Definitions,component instances>>.

=== Port Instance Identifiers

A *port instance identifier* identifies a port instance
that is part of a topology or component instance.
...
a component instance or _I_ or an imported topology _T_.

.. If _Q_ refers to a component instance _I_
