...
If stem:[T_2] is a vector type and stem:[T_1 = T_2], then
let stem:[v_2 = v_1].

Otherwise if stem:[v_1] is an array value, anonymous array value, or vector
value with stem:[k] elements, and stem:[T_2] is a vector type whose maximum size
is at least stem:[k], then
...
. Let stem:[v_2] be the vector value of type stem:[T_2] and length stem:[k]
with value stem:[w_i] at each element stem:[i].
