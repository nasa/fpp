...
with element type _T_.
If the type is a fixed-size array type, then let _n_ be its size.
If the type is a variable-size array type, then let _n_ be its maximum size.
...
_m_ must be greater than or equal to zero.
For a fixed-size array type, _m_ must be less than the size _n_.
For a variable-size array type, _m_ must be less than the
<<Values_Array-Values,length>> of the array value selected by stem:[e_1],
which is at most the maximum size _n_.
