...
where _R_ is one of the following:

1.  An
<<Lexical-Elements_Identifiers,identifier>>.

2.  _R_ `.` _I_, where _R_ is a qualified identifier.
...
. The *interface instance name group*
...
* Component instance definitions reside in the component instance name group and the interface instance name group.
...
* Topology definitions reside in the topology name group and the interface instance name group.
...
A fully qualfied identifier is a special type of qualified identfier
that begins with `.`. This type of identifier 

Name shadowing can happen if a definition in an inner scope overlaps the
unqualified name of a definition in an outer scope. For example:

[source,fpp]
----
constant a = 0
module M {
  constant a = 1 # M.a shadows a
}
----

In this case, `a` is shadowed by `M.a` while inside the module `M`.
To refer to the global `a`, a global qualifier is needed to disambiguate
the name:
...
module M {
  ...
  constant b1 = a # Refers to M.a (1)
  constant b2 = M.a # Refers to M.a (1)
  constant b3 = .a # Refers to a (0)
}
----
...
. If a qualfied identifier is of the form `.` _Q_, where _Q_ is a qualfied identifer,
Do the following:

.. Find the definition _D_ with the fully qualfied name _Q_.
   If there is none, issue an error.
