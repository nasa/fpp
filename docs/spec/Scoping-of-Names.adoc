...
* Topology definitions reside in all name groups.
...
Topology definitions allow <<Definitions_Topology-Definitions_Semantics,some>>
definitions inside. Definitions inside topology definitions are qualified by
...
topology T {
  import M.T
  command connections instance M.T.commandDispatcher
}
----
