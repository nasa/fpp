...
for name scoping>>.
A qualified identifier type name _Q_ names a type a follows:
...
<<Specifiers_Template-Expansion-Specifiers,type template parameter 
definition>>, and _Q_ names the type defined in the global definition
<<Definitions,represented by>> _B_.
