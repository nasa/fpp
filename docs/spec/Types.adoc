...
A *vector type* is a type associated with a
<<Definitions_Vector-Definitions,vector definition>>.
There is one vector type per vector definition.
A vector type has an *element type* _T_, a *maximum size* _n_, and a
*size-prefix type* _Tp_.
A value of a vector type has a *length* in the range [0, _n_], which is the
number of elements that it stores; each element has type _T_.
The maximum size _n_ is a constant integer value, computed after resolving
constants.
The size-prefix type _Tp_, whose <<Types_Underlying-Types,underlying type>> is
an unsigned <<Types_Primitive-Integer-Types,primitive integer type>>, specifies
the type used to serialize the length of the vector (see
<<Types_Serialized-Sizes,serialized sizes>>).
A vector type is written using its
<<Scoping-of-Names_Names-of-Definitions,unique qualified name>>.
...
==== Anonymous Vector Types

The type _[_ `size` _n_ _]_ _T_, where _n_
is an integer and _T_ is a type, represents a vector of at most _n_ elements,
each of type _T_.
The type _T_ is called the *element type*, and the integer _n_ the
*maximum size*, of the anonymous vector type.
An anonymous vector type does not record a size-prefix type; the size-prefix
type of a vector is a property of the
<<Definitions_Vector-Definitions,vector definition>> and is used only when
...
* A <<Types_Vector-Types,vector type>> whose element type is a displayable type.
...
* A <<Types_Vector-Types,vector type>> or
<<Types_Internal-Types_Anonymous-Vector-Types,anonymous vector type>> whose
element type has numeric members.
...
<<Types_Vector-Types,vector type>> _T_ is (1)
the default value specified in the vector definition,
if one is given; otherwise (2) the empty vector, i.e., the value
...
* If _T_ is a
<<Types_Vector-Types,vector type>> with element type _T'_, maximum size _n_,
and size-prefix type _Tp_, then _s_ is the sum of the following two numbers:

** The serialized size of _Tp_.
This is the space used to store the length of the vector.

** The number _n_ times the serialized size of _T'_.
This is the maximum space required to store the elements of the vector.
