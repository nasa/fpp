...
There is one alias type per type definition.
An alias type is written using its
<<Scoping-of-Names_Names-of-Definitions,unique qualified
name>>.

Alias types due not have literal values associated with them.
Values associated with their underlying
<<Types_Canonical-Types,canonical type>> can be casted into
the alias type.
...
A canonical type is the opposite of an <<Types_Alias-Types,alias type>>.
The canonical type is the underlying type with all the "`sugar`" removed.
For example, if `T` is a alias for `U8`, the underlying canonical type for
`T` would be `U8`.

All types can be traced back to a single unique canonical type known as its
<<Types_Underlying-Types,underyling type>>

==== Underlying Types

An underlying type is a <<Types_Canonical-Type, canonical type>> assosicated with
a type. If a type is already a <<Types_Canonical-Type, canonical type>>, it's underlying
type is itself.

The underying type of a <<Types_Alias-Type, alias type>> it's found by recursively stripping
off aliases until the base <<Types_Canonical-Type, canonical type>> is reached.
...
A type *has numeric members* if its <<Types_Underlying-Types,underyling type>>
