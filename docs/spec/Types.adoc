...
The parent type is the type pointed to by the <<Types_Alias-Types,alias type's>> definition.
This relationship creates a tree of types where nodes only know about their
parent node. An ancestor list of a type is a list containing the type
itself as well as the ancestor list of the parent type (given it has a parent).
