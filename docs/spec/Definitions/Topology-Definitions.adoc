...
. Let _S_ be the set of all topologies or template interface parameters that
...
.. If _I_ refers to a <<Definitions_Module-Template-Definitions,template interface parameter>> _T~P~_ inside template _T~D~_ where _T_ is found in expansion _T~E~_:
...
. Replace all instances <<Definitions_Module-Template-Definitions,template interface parameter>> with the
