...
*Special numbering:* The analyzer applies the following special numbering 
passes:

. *Command registration:* Renumber the command registration connections
going into and out of the command dispatcher.

. *Command sequences:* Renumber the command connections out of the 
command sequencer
and the command response connections into the sequencer.

. *Health pings:* Renumber the ping input and ping output connections
of the Health component instance.

In each case, the numbering algorithm ensures that the port number
of each input connection matches the port number of the corresponding
output connection.
