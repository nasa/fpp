...
<<Specifiers_Topology-Import-Specifiers,imported topology>>.

* If _I_ is a component instance, _p_ is an identifier that names a port instance specified in
the component definition associated with _I_.

* If _I_ is topology definition, _p_ is an identifier that names an <<Specifiers_Port-Export-Specifiers,exported port>> inside
_I_. _I_ must be <<Specifiers_Topology-Import-Specifiers,imported>> into the parent topology of the connection.
