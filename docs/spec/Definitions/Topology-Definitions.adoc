...
. If the optional `implements` keyword is present, then check the
following for each qualified identifier _Q_ that appears
qualified identifier appearing after `implements`:

..  _Q_ <<Scoping-of-Names_Resolution-of-Qualified-Identifiers,refers to>> a
