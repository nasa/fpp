...
.. A topology is imported either explicitely through an
<<Specifiers_Topology-Import-Specifiers,import specifier>>
or implicitely through a <<Specifiers_Interface-Instance-Specifiers,
instance specifier>>.
...
.. A direct import is an import in which an <<Specifiers_Topology-Import-Specifiers,
import specifier>> or <<Specifiers_Interface-Instance-Specifiers,instance specifier>>
inside _D_ directly refers another topology.

. Compute the set _S_ of topologies transitively imported into _T_.

.. For each topology stem:[T_i] in _U_:

... Compute the set stem:[U_i] of topologies directly imported into stem:[T_i].

.. The set _S_ is the set union of all stem:[U_i]
...
instances present on interface instances available in _T_.
