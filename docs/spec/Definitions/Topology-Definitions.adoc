...
that is, a set of interface instances
and the connections between their port instances.
Topologies can include port instances which can connect to
ports of interface instances inside the topology.
...
make up the enclosing topology's <<Ports_Port-Interfaces_Anonymous-Port-Interfaces_Anonymous-Topology-Port-Interfaces,
anonymous port interface>>. To compute the interface port instance set is computed as follows:

. For each topology port instance specifier with name _p'_ and _port-instance-id_ _I_ `.` _p_ in the topology:

.. Resolve port instance identifier _I_ `.` _p_ to a port instance _P_.

.. Construct new port instance  _P'_ by replacing _p_ with _p'_.
...
verifies that this topology can <<Ports_Binding-of-Port-Interfaces,bind to>> all 
stem:[K_i].
...
in _T_ and _P_ exists in the <<Ports_Port-Interfaces,port interface>> of _I_.
...
**TODO(tumbar)** Should we just directly reference <<Ports_Port-Interfaces,port interfaces>>
here and just call out to <<Ports_Port-Interface-Instances,port interface instance>>?

A *connection endpoint* is _I_ `.` _p_ or _I_ `.` _p_ `[` _n_ `]`, where

. _I_ is an <<Ports_Port-Interface-Instances,
port interface instance>> that is included in the topology via a
<<Specifiers_Interface-Instance-Specifiers,interface instance specifier>> or implicitly
a <<Specifiers_Topology-Import-Specifiers,topology import specifier>>.
