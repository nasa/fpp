...
If present, the optional keyword `deployment` indicates that the topology
is a **deployment topology**.
A deployment topology is the top-level topology of a deployment; it
causes deployment code to be generated.
...
. Let _S_ be the set of all topologies 
<<Specifiers_Port-Interface-Instance-Specifiers_Semantics,represented by port 
interface instance specifiers>> in _D_.
...
. For each 
<<Specifiers_Connection-Graph-Specifiers_Syntax,endpoint>>
...
.. Otherwise _E_ is a <<Specifiers_Connection-Graph-Specifiers_Semantics,bound
parameter endpoint>>, where _Q_ refers to a bound parameter stem:[B = (p, A)],
and _A_ refers to a <<Ports_Port-Interface-Instances,port interface instance>> 
_N_. 

... Check that the port interface of _p_ contains the port instance _I_.

... Replace _B_ with _N_ in the endpoint _E_.
Retain the port number assignment, if any, at _E_ computed in step 1.

... Reapply steps a, b, and c to the endpoint _E_.
...
in the 
https://fprime.jpl.nasa.gov/latest/docs/reference/fpp-json-dict/#framework-definitions-required-by-the-dictionary[F 
Prime dictionary specification].
...
Here we have imported topology `A` with the `instance` keyword, emphasizing
that `A` is to be treated as an interface instance with ports.
We could also have used `import` as in the examples above.
See the section on <<Specifiers_Port-Interface-Instance-Specifiers_Syntax,port
interface instance specifiers>>.

Import resolution produces this topology:
...
module template T(
  # Introduce an instance by template parameter.
  # At this point we don't know what this instance is.
  # All we know is that it has the port interface IOut.
  instance iOut: IOut
) {
  topology T {
    # On expansion, the topology T will have two instances: iOut and cIn.
    # iOut will refer to the bound parameter definition with name iOut.
    # cIn will refer to the component instance cIn.
    instance iOut
    instance cIn
    connections C {
      # Connection goes from the port interface of iOut to
      # the port interface of cIn
      iOut.p -> cIn.p
      # We must connect ports in the interfaces of the instances named.
      # For example, a connection iOut.pOther -> cIn.p is not allowed here,
      # because pOther is not in the port interface of iOut.
      # This is true even if iOut is eventually bound to cOut, which has the
      # output port pOther.
    }
  }
}

# In the expansion, the bound parameter (iOut: IOut, instance cOut) is in scope.
# After resolution, the connection is cOut.p -> cIn.p.
