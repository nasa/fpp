...
. Let _S_ be the set of all topologies named in 
<<Specifiers_Port-Interface-Instance-Specifiers,port interface instance 
specifiers>> in _D_.
A port interface instance specifier _s_ names a topology _T_ if
(a) _s_ refers to the definition _D'_ of _T_ or (b) _s_ refers to
a <<Definitions,bound parameter definition>> stem:[(p, D')], where _p_
is a <<Template-Parameter-Lists,port interface instance template parameter>>,
and _D'_ is the definition of _T_.
...
. For each instance _I_ in set _S_, compute the **included port interface** _P~I~_ of _I_ accessible in _T_:

.. If _I_ refers to a <<Definitions_Component-Instance-Definitions,component instance>>, _P~I~_ is the port interface of the component instance

.. If _I_ refers to a <<Definitions_Topology-Definitions,topology>>, _P~I~_ is the port interface of the topology

.. If _I_ refers to a <<Definitions_Module-Template-Definitions,port interface instance template parameter>> _T~P~_ inside template _T~D~_ where _T_ is found in expansion _T~E~_:

... The corresponding template parameter value _T~V~_ in _T~E~_ must refer to a port interface instance

... _P~I~_ is the port interface specified in _T~P~_

... _P~I~_ must be a <<Ports_Sub-Interfaces,sub-interface>> of _T~V~_

. If an instance _I_ is named twice in _S_ with differing port interfaces _P~I1~_ and _P~I2~_, the set union will be
used to compute _P~I~_.

. Replace all instances <<Definitions_Module-Template-Definitions,port interface instance template parameter>> with the
