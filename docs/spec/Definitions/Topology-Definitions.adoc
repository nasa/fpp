...
Here we have imported topology `B` with the `instance` keyword, emphasizing
that `B` is to be treated as an interface instance with ports.
...
# In the expansion, the bound parameter (iOut: IOut, cOut) is in scope.
