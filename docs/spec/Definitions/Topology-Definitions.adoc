...
In the FPP source, connections can have ports of either component
instances or topologies as their endpoints.
Before generating code, connections with topology ports as endpoints are flattened
into connections between component instance ports.
...
. Let the set of instances of _T_ be the
...
refers to one of the instances computed in the previous step.
...
<<Definitions_Topology-Definitions_Ordering-of-Connections,
...
When a topology _T_ contains a port interface instance specifier `interface` _Q_,
and _Q_ refers to a topology
