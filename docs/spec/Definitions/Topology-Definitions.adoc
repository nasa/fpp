...
A *partially numbered topology* is a topology in which port array index expressions
(a) may or may not be present and (b) are present if and only if they
are specified in the model source.
...
.. Check that the topology definition named in _S_ has not already
been transitively imported into _T_.
For example, if topology _C_ imports _B_ and _B_ imports _A_, then it is
an error for _C_ to separately import _A_.
