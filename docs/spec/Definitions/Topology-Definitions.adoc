...
The set of all <<Specifiers_Topology-Port-Instance-Specifiers,topology port 
instance specifiers>>
make up the enclosing topology's 
anonymous port interface. (TODO)
To compute the interface port instance set is 
computed as follows:
...
verifies that this topology can bind to (TODO) all 
stem:[K_i].
