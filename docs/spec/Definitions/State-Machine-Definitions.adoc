...
. If the optional state machine member sequence _M_ is present,
then the *transitive members* of _M_ are (i) the elements of _M_ and (ii)
...
*Conflicting names:*
Each kind of definition resides in its own namespace, except
that states and junctions both reside in the state namespace.
No two state machine definitions that reside in the same namespace
