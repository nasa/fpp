...
_E_ that enumerates the leaf states of the state machine _S_ defined by _M_.
_E_ is called the *state enum* of _S_.
...
. The representation type of _E_ is the smallest unsigned integer type
that can represent one plus the number of
<<State-Machine-Behavior-Elements_State-Definitions_Static-Semantics,leaf states>> in _S_.

. _E_ contains the following enumerated constants:

.. The constant `__FPRIME_AC_UNINITIALIZED`.

.. For each leaf state _L_ of _S_, a *leaf state constant* represening _L_.
   Its name is the qualified name of _L_, after replacing `.` with `_`.

. The order of the constants is `__FPRIME_AC_UNINITIALIZED` followed
  by the leaf state constants in lexical order.

. Numerical values are assigned to the constants of _E_ in default way
  (0, 1, 2, etc.).

. _E_ has no explicit default value, so the default value of _E_ is the
  implicit default value `__FPRIME_AC_UNINITIALIZED`.
