...
enter expressions>> _e_ appearing in
<<State-Machine-Behavior-Elements_State-Transition-Specifiers,
state transition specifiers>> and
<<State-Machine-Behavior-Elements_Junction-Definitions,junction definitions>>
induces a directed graph called the *transition graph*.
In this graph, _e_
represents an arc from an *initial node* to a *terminal node*.
If _e_ appears in a state transition specifier _S_, then the initial
node is the
<<State-Machine-Behavior-Elements_State-Definitions,state definition>>
immediately enclosing _S_.
If _e_ appears in a junction definition _J_, then the initial node is _J_.
In either case, 
