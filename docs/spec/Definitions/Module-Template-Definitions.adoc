...
. The rules governing the
  <<Template-Parameter-Lists,template parameter list>> of _M_, including the
  <<Scoping-of-Names_Conflicting-Parameter-Names,rules for conflicting
  parameter names>>.

The body of _M_ is analyzed once for each
<<Specifiers_Template-Expansion-Specifiers,template expansion specifier>>
_S_ that refers to _M_.
Each <<Expansion-of-Templates,expanded template>> stem:[E = S { X }] is
analyzed in the same way as the corresponding syntax written out by hand,
subject to the special rules for
<<Scoping-of-Names_Resolution-of-Identifiers_Inside-an-Expanded-Template,
resolving names inside an expanded template>>.
