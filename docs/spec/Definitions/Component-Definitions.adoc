...
The port instance specifiers must satisfy the following rules:

. All names appearing in any general port specifiers and any internal
port specifiers must be distinct.

. No passive component may have an `async` `input` port.

. Each active or queued component must have (a) at least one `async` `input` 
port or (b) at least one command.
