...
The command, event, parameter, and telemetry specifiers
are collectively referred to as *dictionary specifiers*.
...
|There must be specifiers for ports `product` `request`, `product` `recv`,
and `product` `send`.
|===
...
The data product specifiers must satisfy the following rules:
...
active component CommandDispatcher {
  ...
  product recv port productRecvOut
