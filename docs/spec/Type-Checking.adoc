...
The type of a <<Expressions_Sizeof-Expressions,sizeof expression>> is _Integer_.
...
To type a
<<Expressions_Unary-Negation-Expressions,unary
...
To type a <<Expressions_Binary-Expressions,binary expression>> stem:[e_1] _op_ stem:[e_2]
the semantic analyzer does the following:

. If _op_ is `+`, `-`, `*`, or `/`, then use the
  <<Type-Checking_Binary-Expressions_Arithmetic-and-Concatenation,
  rules for arithmetic and concatenation expressions>>.

. Otherwise use the 
  <<Type-Checking_Binary-Expressions_Shifting,
  rules for shift expressions>>.

==== Arithmetic and Concatenation

To type a <<Expressions_Binary-Expressions,binary expression>> stem:[e_1] _op_
stem:[e_2] where _op_ is `+`, `-`, `*`, or `/`, the
semantic analyzer does the following:

. Compute the <<Type-Checking_Computing-a-Common-Type,common type>> stem:[T] of 
  stem:[e_1] and stem:[e_2].

. If stem:[T] is a <<Types_Internal-Types_Numeric-Types,numeric type>>,
then use stem:[T].

. Otherwise if stem:[T] <<Type-Checking_Type-Conversion,may be converted to>>
_Integer_, then use _Integer_.

. Otherwise if _op_ is `pass:[+]` and stem:[T] is a
<<Types_String-Types,string type>>, then use stem:[T].
...
constant f = "ab" + "cd" # Type is String
----

==== Shifting

To type a <<Expressions_Binary-Expressions,binary expression>> stem:[e_1] _op_
stem:[e_2], where _op_ is `<<` or `>>`,
the semantic analyzer does the following:

. For each stem:[i] in stem:[[1,2\]]

.. Compute the type stem:[T_i] of stem:[e_i].

.. Check that stem:[T_i] is an 
  <<Types_Internal-Types_Integer-Types,integer type>>
  or an <<Types_Enum-Types,enum type>>.

. If stem:[T_1] is an integer type, then use stem:[T_1]
...
----
constant a = 1 << 2 # Type is Integer
constant b = 1.0 << 2 # Error: left operand is a floating-point type
constant c = 1 << 2.0 # Error: right operand is a floating-point type
enum E { X = 1 }
constant d = E.X << 2 # Type is Integer
----
...
<<Types_Enum-Types,enum type>>,
<<Types_Struct-Types,struct type>>, or
<<Types_Vector-Types,vector type>>,
...
. If stem:[T_1] or stem:[T_2] or both are
<<Types_Vector-Types,vector types>>, then
stem:[T_1] may be converted to stem:[T_2] if the conversion
is allowed after replacing the vector type or types with
structurally equivalent
<<Types_Internal-Types_Anonymous-Vector-Types,anonymous vector types>>.

. An anonymous array type stem:[T_1 =] _[_ stem:[n] _]_ stem:[T'_1]
may be converted to the anonymous vector type
stem:[T_2 =] _[_ `size` stem:[m] _]_ stem:[T'_2]
if stem:[n <= m] and stem:[T'_1] may be converted to stem:[T'_2].

. An anonymous vector type stem:[T_1 =] _[_ `size` stem:[n] _]_ stem:[T'_1]
may be converted to the anonymous vector type
stem:[T_2 =] _[_ `size` stem:[m] _]_ stem:[T'_2]
if stem:[n <= m] and stem:[T'_1] may be converted to stem:[T'_2].
Note that the size-prefix type of a vector type does not affect convertibility.

. A <<Types_Internal-Types_Numeric-Types,numeric type>>,
<<Types_The-Boolean-Type,Boolean type>>, <<Types_Enum-Types,enum type>>, or
<<Types_String-Types,string type>> is never convertible to a
<<Types_Vector-Types,vector type>> or anonymous vector type, notwithstanding the
transitivity rule below.
Unlike an array value, a vector value is not filled from a single scalar value;
its elements must be given explicitly, for example as an array expression.
...
common type>> stem:[T'_i] of stem:[T'_(i-1)] and stem:[T_i].

.  Use stem:[T'_n] as the common type of the list.
