...
To type a
<<Expressions_Binary-Expressions,unary
...
To type a
<<Expressions_Binary-Expressions,binary
arithmetic expression>> stem:[e_1] _op_ stem:[e_2], the semantic analyzer does
the following:

. If _op_ is `+` and both stem:[e_1] and stem:[e_2] have
<<Type-Names_String-Type-Names,string type>>, then the expression has
string type and denotes
<<Expressions_Binary-Expressions,string concatenation>>.

. Otherwise compute the common type stem:[T] of stem:[e_1] and stem:[e_2].
