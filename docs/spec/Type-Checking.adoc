...
. stem:[T_1] and stem:[T_2]
are both 
<<Types_The-String-Type,the string type>>.
...
type>>,
<<Types_Enum-Types,enum type>>, or <<Types_The-String-Type,string type>> stem:[T] 
may be converted to an
<<Types_Internal-Types_Anonymous-Array-Types,anonymous array type>> stem:[T_2] if
...
. Otherwise if one of stem:[T_1] and stem:[T_2] is an array type
and the other one is an array type or anonymous array type, then 
replace the array type or types with structurally equivalent anonymous array
types and reapply these rules.
...
. Otherwise if one of stem:[T_1] and stem:[T_2] is an struct type
and the other one is an struct type or anonymous struct type, then replace the 
struct type or types with structurally equivalent anonymous struct
types and reapply these rules.
...
the
primitive type and each of the struct member types to a common type.
...
. Otherwise if one of stem:[T_1] and stem:[T_2] is a primitive type 
and the other
one is a struct type stem:[S], and the primitive type
