...
. Otherwise if one of stem:[T_1] and stem:[T_2] is a struct type
and the other one is a struct type or anonymous struct type, then replace the 
...
convertible to an anonymous array type
and the other
one is an array type stem:[A], and stem:[T']
...
. Otherwise if one of stem:[T_1] and stem:[T_2] is a type stem:[T'] that is
convertible to an anonymous struct type and the other
one is a struct type stem:[S], and stem:[T]
