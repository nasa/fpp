...
* An *interface instance location specifier* `locate` `instance`
...
* A *template specifier* `locate` `template`
<<Scoping-of-Names_Qualified-Identifiers,_qual-ident_>> `at`
<<Expressions_String-Literals,_string-literal_>>

* A *system location specifier* `locate` `system`
<<Scoping-of-Names_Qualified-Identifiers,_qual-ident_>> `at`
<<Expressions_String-Literals,_string-literal_>>
...
.. An interface instance location specifier refers to a
...
.. A system location specifier refers to a
<<Definitions_System-Definitions,system definition>>.

.. A template specifier refers to a 
   <<Definitions_Module-Template-Definitions,template definition>>.

.. A type location specifier refers to a <<Definitions_Type-Definitions,type 
   definition>>.

. When a location specifier appears inside a
<<Definitions_Module-Definitions,module definition _M_>>
and does not begin with a dot, _Q_ is implicitly qualified by the
