...
* The optional expression _d_ following `every` specifies the maximum time period
to elapse before clearing the event throttle. _d_. The type of _d_ must be convertible
to a <<Types_Struct-Types,struct type>> with shape:
