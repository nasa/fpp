...
.. If either specifier is present, then
the type of the
...
.. The type of the expression in each limit must be 
<<Type-Checking_Type-Conversion,convertible to>> 
...
...  If _T_ has numeric members but is not itself a numeric type
(e.g., it is an array), then the limit is applied to each member value of _v_.
