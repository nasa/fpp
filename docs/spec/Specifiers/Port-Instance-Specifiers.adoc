...
==== Input and Output Port Instance Specifiers

The following port instance specifiers are *input port instance specifiers*:

* Any general port with `input` in its name.

* Special ports `command recv` and `product recv`.

The others are *output port instance specifiers*.
...
Each special port instance represents an
<<Definitions-and-Uses_Implied-Uses,implied use>> of a
<<Definitions_Framework-Definitions_Port-Definitions,framework port>>
