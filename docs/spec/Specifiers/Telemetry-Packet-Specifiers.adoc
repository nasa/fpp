...
. FPP resolves any include specifiers in _telemetry-packet-member-sequence_ to a 
list _L_
of telemetry channel identifiers.
It then checks that for each telemetry channel identifier _I_ in _L_,
the component instance referred to in _I_ is
