...
If _Q_ refers to a <<Definitions_Component-Instance-Definitions,component 
instance definition>>, then _E_ is a *component instance endpoint*.
Otherwise if _Q_ refers to a <<Definitions_Topology-Definitions,topology 
definition>>, then _E_ is a *topology endpoint*.
Otherwise _Q_ refers to a <<Specifiers_Template-Expansion-Specifiers,port 
interface instance parameter definition>>, and _E_ is a *bound parameter 
endpoint*.
...
It may refer to the component instance directly, or it may refer to a
<<Specifiers_Template-Expansion-Specifiers,port interface instance parameter
definition>> that <<Ports_Port-Interface-Instances,represents>> a component
instance definition.
Because a pattern graph is resolved after every connection endpoint is a
component instance endpoint, a pattern connects the ports of the component
instance represented by such a parameter, including ports that are not in the
port interface declared for the parameter.

. If the instance sequence _I_ appears, then each qualified identifier
_Q_ appearing in _I_ must refer, in the same way as the qualified identifier
following the keyword `instance`, to a component instance that exists
