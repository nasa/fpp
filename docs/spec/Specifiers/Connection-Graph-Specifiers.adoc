...
identifiers>> _I_ appearing in _E_.

... The port interface instance _P_ named in _I_ must exist in the enclosing 
topology,
after 
...
If _P_ is a component instance, then _E_ is a *component instance endpoint*.
Otherwise _P_ is a topology, and _E_ is a *topology endpoint*.
