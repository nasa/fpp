...
array type.
An anonymous array value is always a fixed-size array value.
A value of a variable-size <<Types_Array-Types,array type>> is always an
<<Values_Array-Values,array value>> whose canonical type is a named array type.
...
. stem:[n] is a nonnegative integer called the *length* of the array value.
If stem:[Q] refers to a fixed-size array definition of size stem:[m], then
stem:[n = m].
If stem:[Q] refers to a variable-size array definition of maximum size stem:[m],
then stem:[0 <= n <= m]; in particular, the length may be zero, in which case
...
* Otherwise if _v_ is a fixed-size <<Types_Array-Types,array type>>, then _s_ is
  the sum of the serialized sizes of the elements of _v_.

* Otherwise if _v_ is a variable-size <<Types_Array-Types,array type>> with
  size-prefix type _Tp_, then _s_ is the sum of the following two numbers:
