...
* A <<Template-Parameter-Lists,port interface instance template parameter>>.

* A <<Specifiers_Template-Expansion-Specifiers_Semantics,port interface
...
==== Port Interface Instance Template Parameters

For every <<Template-Parameter-Lists,port interface instance template
parameter>> _P_, there
is an associated port interface _I_.
It is the port interface associated with the
<<Ports_Port-Interfaces_Port-Interface-Definitions,port interface definition>>
referred to in _P_.

==== Port Interface Instance Parameter Definitions

For every <<Specifiers_Template-Expansion-Specifiers_Semantics,port interface
instance parameter definition>> stem:[(p, A)], there is an associated
port interface _I_.
It is the port interface associated with the
<<Ports_Port-Interfaces_Port-Interface-Instance-Template-Parameters,port
interface instance template parameter>> _p_.
_I_ is a <<Ports_Sub-Interfaces,sub-interface>> of the port interface of the
<<Ports_Port-Interface-Instances,port interface instance>> to which _A_ refers.
...
. A <<Specifiers_Template-Expansion-Specifiers_Semantics,port interface 
instance parameter definition>> stem:[(p, A)], where
...
. For a <<Specifiers_Template-Expansion-Specifiers_Semantics,port interface
  instance parameter definition>> stem:[(p, A)], the port
  interface is given by the
  <<Ports_Port-Interfaces_Port-Interface-Instance-Template-Parameters,template
  parameter>> _p_.
