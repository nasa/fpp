...
. A <<Definitions,bound parameter definition>> stem:[(p, T)], where
_p_ is a <<Template-Parameter-Lists,port interface instance template 
parameter>>, and _T_ is a topology definition.
...
. For a bound parameter definition stem:[(p, T)], the port interface is specified
  in the parameter _p_. In general it is a subset of the port interface
  of _T_.
