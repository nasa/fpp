...
values for the structure members.
...
*XML limitation:*
FPP structs are translated to F Prime Serializable XML types.
The F Prime Serializable format does not support complex types
in struct members.
When translating to F Prime XML, you should use only simple types
in struct members.
If you write a complex type in a struct member, it will pass the
FPP analysis and FPP will generate XML, but the resulting XML
will not be handled correctly by the F Prime XML autocoder.
...
code, and therefore the order in which the members are serialized to binary data.
...
to be generated in files `A.hpp` and `A.cpp`.
* File `A.hpp` includes a header file `T.hpp`.
...
<<Translating-Models,translating models>>.
