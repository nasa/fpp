...
array values>>,
an array type must have size greater than zero and less than or equal to a 
maximum that is implementation-dependent
...
==== The Optional Member Format Strings
...
Its purpose is to bring the name _N_ into scope and to tell the analyzer that a 
type with name _N_ exists and will be defined elsewhere.
For example, if the target language is {cpp}, then the type may be a {cpp} 
class.
