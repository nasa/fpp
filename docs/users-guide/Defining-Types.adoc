...
`T` is an alias for the type `U32`:
...
another alias.
For example, here is an alias type definition specifying that the type
`T` is an alias for the struct type `S`:
...
Here is a set of definitions specifying that `S` is an alias for `T`,
and `T` is an alias for `F32`:
...
Here is set of alias type definitions that is illegal, because each of
