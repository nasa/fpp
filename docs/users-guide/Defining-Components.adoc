...
As an example, here is an active component called `SimpleEvents`
...
As for command opcodes, if you omit the identifier, then
...
If you wish, you may explicitly specify one or more identifiers.
...
After the component has emitted the event _n_ times, it will
...
has emitted it ten times.

Once an event is throttled, the event will not be emitted again
until the throttling is canceled.
