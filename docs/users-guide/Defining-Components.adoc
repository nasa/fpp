...
example above and running the result through `fpp-check`.
...
When a command is sent to the application, or dispatched
to a component, the command is identified by its opcode.
The command name is mainly for purposes of human interaction
on the ground.
For sending and dispatching commands, the opcode is used,
because it is more compact than the sequence of characters
representing the name.

When specifying a command, you specify its opcode
relative to the component where the command is defined.
Typically the opcodes for each component start at zero: that is, the
opcodes are 0, 1, 2, etc.
When constructing a component instance, you further resolve the opcode by 
providing a base opcode for the instance;
this base opcode is added to the relative opcodes to form
the final set of opcodes for the instance.
We will have more to say about this issue in the section on constructing
topologies of component instances.

If you don't specify an opcode for a command, as in the examples
in the previous sections, then a default opcode is used.
The default opcode for the first command in the component is zero.
Otherwise the default opcode for any command is one more than
the opcode of the previous command.

To specify an opcode explicitly, you write the keyword `opcode` followed
by a numerical expression after the command name and after the
