...
==== External Parameters
F Prime parameters are typically stored and managed internally to the
component, in the autocoded base class.
Parameter values can be accessed by the component implementation through
autocoded get functions.
Internal parameters are defined without any prefix.

In some cases, a parameter may need to be managed and stored outside
the component.
This can be done by declaring a parameter to be external, using the
`external` keyword prefix.

Here is an example of an internal parameter, `Param1`, and an external parameter, `Param2`:
...
passive component ParamExternal {
  ...
  @ Parameter 1
  @ Its implied to be an internal parameter
  param Param1: U32

  @ Parameter 2
  @ Its an external parameter
...
Note that an external parameter can still specify
<<Defining-Components_Parameters_Default-Values,default values>>,
<<Defining-Components_Parameters_Identifiers,identifiers>>, and
<<Defining-Components_Parameters_Set-and-Save-Opcodes, set/save opcodes>>
in the same manner as an internal parameter.

===== External Parameter Delegate

A component with externally managed parameter(s) must still be able
to perform the nominal set/save/get parameter actions.
To facilitate this the component needs the ability to serialize
and deserialize a parameter value.
This is facilitated by implementing the interface defined by the
`ParamExternalDelegate` abstract base class, and registering the
concrete implementation by calling `registerExternalParameters()`
during FSW initialization.
`registerExternalParameters()` can be called directly in the
component implementation constructor, or exposed as a public
interface to be called by the topology.

S concrete implementation of the `ParamExternalDelegate` abstract
base class is autocoded in the component tester base.
This implementation can be used for unit testing or another user
defined concrete implementation can be used.
Either option must call the `registerExternalParameters()`
function during FSW initialization.
