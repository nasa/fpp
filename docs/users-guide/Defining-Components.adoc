...
*XML limitation:* The F Prime XML format does not support
...
in `Commands.fpp`, just as if you had written them
...
specifier appears.
For example, if `Commands.fpp` contains invalid syntax
or syntax that may not appear inside a component,
or if the file `Commands.fpp` does not exist, then
the specifier `include "Commands.fpp"` is not valid.
