...
instance.
We use the keyword `port` to denote a port instance.
The kind goes first, followed by the keyword `port`, the port instance
name, a colon, and the port definition.
...
distinction between a _port_ (defined outside any component)
...
We will occasionally do that in this manual.
For example, we may say that the `F32Adder` component has three
ports: two async input ports and one output port.
...
Third, a port definition appearing in an `async` `input` port
...
passive component F32Adder {
  ...
  @ Inputs 0 and 1
  sync input port f32ValueIn1: [2] F32Value
