...
* Otherwise, let _o_ be the previous opcode in the component
...
*XML limitation:* In general, a qualified name such as `Switch.State`
in FPP becomes a qualified name such as `Switch::State` when translating
to {cpp}.
However, the F Prime XML format does not currently support the declaration
of constants and types as members of components.
