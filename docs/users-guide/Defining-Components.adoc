...
==== Displayable Types
Displayable types are those that contain all the information that 
the Ground Data System requires to display the type. 
When defining a command, telemetry channel, event, or parameter,
ensure that any types appearing in the specifier are displayable. 
For more information about displayable types, see the https://nasa.github.io/fpp/fpp-spec.html[_The FPP Language Specification_].

In the event a non-displayable type is used in a command, telemetry channel, event, or parameter, the FPP compiler will produce an error such as:

----
% fpp-check
  async command C(a1: S)
                      ^
error: command parameter type must be displayable
----
In this example, the command formal parameter `a1` is not a displayable type. 
...
<<Defining-Components_Commands_Displayable-Types, displayable types>>.
