...
A *state machine instance* is a component member that specifies
a named instance of an FPP
<<Defining-State-Machines,state machine>>.
It says that the state machine instance is part
...
A state machine instance _m_ inside a component _C_
processes *signals* as it runs.
Signals occur in response to commands or port
input to _C_, and they tell _m_ when to change state.
The command handlers and input handlers of _C_ put signals on 
the queue of _C_,
and _m_ dispatches them off the queue.
Therefore, a component with a state machine instance
member must have a queue, i.e., it must be active or queued.

As with <<Defining-Components_Internal-Ports,internal ports>>,
you may specify priority and queue full behavior.
