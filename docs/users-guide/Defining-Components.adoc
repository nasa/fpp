...
When writing a telemetry channel, you can specify how
often the the telemetry is emitted.
There are two possibilities:

* `always`: Emit a telemetry point on the channel
whenever the component instance calls the corresponding
telemetry function.

* `on` `change`: Emit a telemetry point whenever
(a) the component instance calls the corresponding
telemetry function and (b) the telemetry value
is different from the previous value emitted
on that channel.

Emitting telemetry on change can reduce unnecessary
activity in the system.
For example, suppose a telemetry point counts
the number of times that some event has occurred
in a periodic task,
and suppose the event occurs relatively rarely.
Always emitting the telemetry will cause a new
telemetry point to be updated (and possibly logged,
transmitted to the ground, etc.) on every cycle.
Emitting the telemetry on change will cause an update
only when there is a new value.
...
When writing a telemetry channel, you may specify how the channel
is formatted.
...
values are expected to stay above the limit) and `high`
(meaning that the values are expected to stay below the limit).
...
The ground data system can display an appropriate warning
