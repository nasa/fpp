...
For the use of `+` with string operands, see
<<Defining-Constants_Expressions_String-Concatenation,String Concatenation>> below.

[[Defining-Constants_Expressions_String-Concatenation]]
==== String Concatenation

You can use the `+` operator to concatenate two string values.
Both operands must have string type. The result is a string formed
by appending the second operand to the first. For example:
...
Since `+` is left-associative, `"a" + "b" + "c"` is evaluated as
`("a" + "b") + "c"`.
