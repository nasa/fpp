...
==== Value Bitwise Shift Expressions
A *value bitwise shift expression* performs bitwise shift on values.
It is one of the following:

* A left shift expression using `<<`, for example:
+
[source,fpp]
----
constant a = 1 << 2
...
* A right shift expression using `>>`, for example:
+
[source,fpp]
----
constant a = 8 >> 1
...
<<Defining-Constants_Expressions_Value-Arithmetic-Expressions,addition>>,
except that it operates on strings instead of numbers.
For example:

[source,fpp]
----
constant a = "hello " + "there!" # value of a is "hello there!"
----
...
You can also use parentheses in the usual way to group subexpressions.
