...
required to represent any value of the specified type. 
For example:
[source,fpp]
----
...
*Sizeof string type expressions:* When writing a `sizeof(string)` expression,
you must define the `FwSizeStoreType` framework type. Additionally, if 
you do not include a size when writing the string type, you must also
define the `FW_FIXED_LENGTH_STRING_SIZE` framework constant. These 
framework definitions are used when computing the sizeof a string type.

Here is an example of a string without a string size specified:
...
Here is an example with a user specified string size:

[source,fpp]
----
type FwSizeStoreType = U16
constant s = sizeof(string size 20)
----

In this example, the constant `s` has a value of 22.
