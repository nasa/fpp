...
The member of an array can be selected through an *array subscript expression*.
...
----
constant s = [ 3, 2, 1 ]
constant o = s[1] # equals 2
----

This code associates the name `s` with an array value and another name `o` with
an element of `s`. The final value of `o` is `2`. The index in array subscript
expressions are `0` indexed meaning the first element exists at index `0`.
...
A member of a struct may be selected through a `.` operator.
For example:
...
This code associates `s` with a struct value with members `x` and `y`. It also
associates `t` with the `x` member of `s`. `t` will have the value `1`.
