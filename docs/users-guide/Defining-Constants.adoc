...
application; the computations are specified in a separate implementation 
...
that is up to the implementation language.
For example, if the implementation language is {cpp}, then array indices start 
at zero.
...
This code is also allowed, and it is equivalent to the previous code:
...
For example, one can do a line-by-line sort on the elements (for example, to 
