...
To encode the character `\` followed by the character `q`, write
the backslash character as `\b`, like this:
...
This string represents the literal character sequence `"`, `\`, `q`, `\`, `b`.
...
As in a single-line string, `\b` means `\`.
