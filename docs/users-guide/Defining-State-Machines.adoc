...
*Special cases:*
The behavior described above agrees with the behavior
that we described in the section on 
<<Defining-State-Machines_Actions_Entry-and-Exit-Actions,
...
This behavior also agrees with the behavior that we described for
<<Defining-State-Machines_More-on-State-Transitions_Self-Transitions,
self transitions>>.
When _S1_ and _S2_ are the same state _S_, the least common
ancestor _N_ of _S1_ and _S2_ is the parent of _S_.
So we exit _S_ to go up to _N_, do the actions of _T_, and
reenter _S_.
