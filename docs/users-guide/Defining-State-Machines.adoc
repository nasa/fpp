...
_S1_ to _S2_ generates exactly one flattened transition,
also from _S1_ to _S2_.
...
that "`looks like`" it goes from a state _S1_ to a state
_S2_, but, if _S1_ is a parent state, actually represents
several flattened state transitions.
For each leaf descendant _L_ of _S1_, there is a flattened
state transition from _L_ to _S2_.
...
always goes from a state _S1_ to a state _S2_, both of which are
...
case of a flattened state transition _T_ from a state _S1_ to a state _S2_,
where _S2_ is an ancestor of _S1_.
...
contains a syntactic transition _Ts_ that enters _S_.
Then _Ts_ looks like a self transition (it is defined in _S_,
and its target is _S_).
However, by the rules for
<<Defining-State-Machines_Hierarchy_Inherited-Transitions,inherited 
transitions>>,
_Ts_ in fact represents one or more flattened state transitions _T_,
each of which has a leaf descendant of _S_ as its source, and so
is an ancestor state transition.

* We do not have to consider the case where _S1_ is an ancestor
of _S2_: if _T_ is flattened, then _S1_ is a leaf state,
and leaf states have no descendants.

When _T_ is an ancestor state transition, the least common ancestor
of _S1_ and _S2_ is the parent _P_ of _S2_.
Therefore the following behavior occurs:

. Do exit actions to get from _S1_ to _P_. The last actions will
be the exit actions of _S1_, if any.

. Do the actions of _T_, if any.

. Do enter actions to get from _P_ to _S2_.
The first actions will be the entry actions of _S1_, if any.
