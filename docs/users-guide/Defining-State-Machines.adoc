...
The state machine implementation must be provided by an
external tool.
...
it provides a complete specification of the state machine behavior,
which the FPP back end can use to generate code.
...
the state machine `M` inside a component definition.
