...
Note that a sate is never an ancestor of itself.
...
_S_.
Note that a sate is never a descendant of itself.
...
For example, it is possible to write a syntactic state transition
that "`looks like`" it goes from a state _S_~1~ to a state
_S_~2~, but, if _S_~1~ is a parent state, actually represents
several flattened state transitions.
For each leaf descendant _L_ of _S_~1~, there is a flattened
...
We now describe the more general behavior.
...
occurs from state _S_~1~ to state _S_~2~.
By the definition of a flattened state transition, we may assume
that _S_~1~ is a leaf state.
When carrying out the transition _T_, the state machine instance will do
actions as follows:

. Compute the *least common ancestor* of _S_~1~ and _S_~2~.
This is the unique node _N_ of the <<Defining-State-Machines_Hierarchy_Substates,
hierarchy tree>> of _M_ such that (a) _N_ is an ancestor of _S_~1~,
(b) _N_ is an ancestor of _S_~2~, and (c) there is no node _N'_
that is a descendant of _N_ and that satisfies properties (a) and (b).

. Traverse the hierarchy tree upwards from _S_~1~ to _N_.
...
. Traverse the hierarchy tree downwards from _N_ to _S_~2~.
...
_A_ and _B_, _A_ has substate _A'_, and _B_ has substate _B'_.
Suppose that _T_ goes from _A'_ to _B'_.
Then least common ancestor is _S_, and the following actions
would be done, in the following order:
the exit actions of _A'_, the exit actions of _A_,
the actions of _T_, the entry actions of _B_, and the
entry actions of _B'_.

Remember also that if _B'_ is not a leaf state, then _T_ will
...
https://github.com/nasa/fpp/wiki/Checking-Semantics[FPP wiki].
...
When a state machine _M_ has no hierarchy, a flattened state transition _T_
always goes from a state _S_~1~ to a state _S_~2~, both of which are
children of _M_ in the hierarchy tree.
So we always exit _S_~1~, do the actions of _T_, and enter _S_~2~.

The general behavior also agrees with the behavior that we previously
described for
<<Defining-State-Machines_More-on-State-Transitions_Self-Transitions,
self transitions>>.
When _S_~1~ and _S_~2~ are the same state _S_, the least common
ancestor of _S_~1~ and _S_~2~ is the parent _P_ of _S_.
So we exit _S_ to go up to _P_, do the actions of _T_, and
reenter _S_.

It is also important to consider and understand the special
case of a flattened state transition _T_ from a state _S_~1~ to a state _S_~2~,
where _S_~2~ is an ancestor of _S_~1~.
We call this kind of transition an *ancestor state transition*.
Note the following:

* An ancestor state transition is not a self transition, because
a state is never an ancestor of itself.

* Suppose that some state _S_ is a parent state, and the definition of _S_ 
contains a syntactic transition _T~s~_ that enters _S_.
Then _T~s~_ looks like a self transition (it is defined in _S_,
and its target is _S_).
However, by the rules for
<<Defining-State-Machines_Hierarchy_Inherited-Transitions,inherited 
transitions>>,
_T~s~_ in fact represents one or more flattened state transitions _T_,
each of which has a leaf descendant of _S_ as its source, and so
is an ancestor state transition.

* We do not have to consider the case where _S_~1~ is an ancestor
of _S_~2~: if _T_ is flattened, then _S_~1~ is a leaf state,
and leaf states have no descendants.

When _T_ is an ancestor state transition, the least common ancestor
of _S_~1~ and _S_~2~ is the parent _P_ of _S_~2~.
Therefore the following behavior occurs:

. Do exit actions to get from _S_~1~ to _P_. The last actions will
be the exit actions of _S_~1~, if any.

. Do the actions of _T_, if any.

. Do enter actions to get from _P_ to _S_~2~.
The first actions will be the entry actions of _S_~1~, if any.
...
Another way to say this is that (since no choice cycles are
allowed) each transition path out of _I_ must go through
zero or more choices with parent _P_
...
state machine ValidChoice {
  ...
  state S1 {
    ...
    @ This choice is valid: C, S2, and S3 all have parent S1
...
state machine InvalidChoice {
  ...
  state S1 {
    ...
    @ This choice is invalid: C has parent S1, but S2 and S3 have parent InvalidChoice
    choice C { if g enter S2 else enter S3 }
