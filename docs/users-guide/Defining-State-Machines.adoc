...
several flattened state transitions, one from each of the descendants
of _S1_ to _S2_.
...
transition occurs.
In fact all the sequences of actions for flattened state transitions
can be computed before generating code, and it is more efficient
to do this.
...
transitions>>,
in fact it represents one or more flattened state transitions _T_,
each of which is an ancestor state transition.
