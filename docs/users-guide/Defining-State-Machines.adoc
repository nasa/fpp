...
. If some transitions into _C_ carries no value,
then neither of the transitions out of the choice carries a value.
...
*parent* of _T_, and _T_ is called a *substate* of _S_.
...
This lets us express the state machine structure in a
disciplined and modular way.

. Define behaviors of a parent state that are inherited
by its substates.
This saves having to redefine the behavior for each
substate.
