...
For example, if _S_~1~ is a parent state, then a syntactic state transition
that from a state _S_~1~ to a state
_S_~2~ may represent several flattened state transitions.
For each descendant _L_ of _S_~1~ that is a leaf state, there is a flattened
