...
<<Defining-State-Machines_Entry-and-Exit-Specifiers,
...
<<Defining-State-Machines_Self-Transitions,self transition>>,
...
* The name of a <<Defining-State-Machines_Guarded-Transitions,guard>>
_G_.
...
<<Defining-State-Machines_Typed-Signals-and-Actions,
carry a value>>.
...
<<Defining-State-Machines_States-Transitions-and-Signals, a state machine 
without hierarchy>>.
...
<<Defining-State-Machines_Hierarchy_Substates_Rules-for-Substates,
leaf states>> that is a
<<Defining-State-Machines_Hierarchy_Substates_Ancestors-and-Descendants,
descendant of _T_>>.
...
and a state _S'_ under _S_ defines another transition _T'_
...
Here we have rewritten the `Device` state machine from the previous section
so that all the states in that example are now substates of a single state
`DEVICE`.
...
<<Defining-State-Machines_Internal-Transitions,internal transition>>
...
==== Transitions Across State Boundaries

TODO

==== Transitions to Ancestor States
