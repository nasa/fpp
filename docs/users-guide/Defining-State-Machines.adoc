...
self transitions>>,
we explained how entry and exit actions occur for a state transition
in a state machine without hierarchy.
That behavior is a special case of a more general behavior
for state machines with hierarchy.
...
Remember that _T_ must 
<<Defining-State-Machines_Hierarchy_Substates,go to a leaf state>>,
following initial transitions if necessary.
Any actions specified in those transitions are done as well,
after the transitions described above, and in the order that
the initial transitions are run.

Notice also that this behavior agrees with the more specific
