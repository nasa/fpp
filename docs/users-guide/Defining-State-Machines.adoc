...
carry a value>>.
To determine whether a choice _C_ carries a value, and if so
what type it has, we use the following rules:

. If any of the transitions going into _C_ carries no value,
then neither of the choice transitions carries a value.

. Otherwise if all of the incoming transitions carry
a value of the same type _T_, then each of the choice
transitions carries a value of type _T_.

. Otherwise if the incoming types can be resolved to
a common type _T_, then each of the transitions carries
a value of type _T_.
...
In this case, the type named in the action definition may be
compatible with the type associated with the transition,
as discussed above.
...
** Initial transition specifier must go through peer choices to a peer state
