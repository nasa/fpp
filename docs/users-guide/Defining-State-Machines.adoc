...
An *action* is a function that a state machine calls when it takes
a transition.
...
To specify that an action should be run, you write the keyword `do`
...
`initialAction1`, `initialAction2`, `onAction`, and `offAction`.
...
To do an action means to call the corresponding action function.
