...
topology definitions and the main function.
...
As part of the {cpp} implementation, you must write the file
_T_ `TopologyDefs.hpp`, following the description below.
It must be located in the same directory where the topology _T_ is defined.
...
`hostName` that stores a host name and U32 value `portNumber` that stores
a port number.
...
The generated code passes the `state` object into the topology setup and
teardown functions.
The `startTasks` phase for the `comm` function uses it in the following way:
...
handling signals, and returning an exit code to the system on exit.
