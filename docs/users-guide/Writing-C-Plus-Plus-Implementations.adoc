...
+
----
Fw::SerializeStatus T::serializeTo(
  Fw::SerialBufferBase&,
...
+
----
Fw::SerializeStatus T::deserializeFrom(
  Fw::SerialBufferBase&,
  Fw::Endianness = Fw::Endianness::BIG
)
----
...
two bytes `00` `0A` (hex). If little-endian order is desired, the optional mode 
parameter can be specified as `Fw::Serialization::LITTLE`. This stores the data 
least significant byte order. The `U16` value 10 (decimal) is serialized in 
little-endian as the two bytes `0A` `00` (hex).

.. A value of signed integer type (`I8`, `I16`, `I32`, or `I64`)
is serialized by first converting the value to an unsigned value of the same 
bit width and then serializing the unsigned value as stated in rule 1.a.
...
... The `I16` value 10 (decimal) is serialized as two bytes, yielding the bytes 
`00` `0A` (hex) in big-endian and `0A` `00` (hex) in little-endian.
...
element, in array order, with no padding.
...
with no padding.
Each member is serialized as follows:

.. If the member is not a
<<Defining-Types_Struct-Type-Definitions_Member-Arrays,member array>>,
then it represents a single value with a single type.
In this case the value is serialized according to its type as specified
in these rules.

.. Otherwise the member is a
<<Defining-Types_Struct-Type-Definitions_Member-Arrays,member array>>,
which represents stores an array of typed values.
In this case, the array is serialized as specified in rule 5, as if it
were a single value of an <<Defining-Types_Array-Type-Definitions,array type>>.
...
.. If the member is not a
<<Defining-Types_Struct-Type-Definitions_Member-Arrays,member array>>,
then it is a single value of an FPP type.
In this case the value is serialized according to its type as specified
in these rules.
Note that the type of the value may be an
<<Defining-Types_Array-Type-Definitions,array type>>.

.. Otherwise the member is a
<<Defining-Types_Struct-Type-Definitions_Member-Arrays,member array>>,
which is not a single value, but an array of typed values.
In this case, the array is serialized as specified in rule 5,
as if it were a single value with an 
<<Defining-Types_Array-Type-Definitions,array type>>.
