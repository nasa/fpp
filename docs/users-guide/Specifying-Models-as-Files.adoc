...
. Run `fpp-to-fpp -i b.fpp`.

`fpp-to-fpp` is a tool for rewriting FPP source files.
It also can expand include specifiers.
`fpp-to-fpp` is discussed further in the section on
<<Analyzing-and-Translating-Models_Rewriting-FPP-Source,
