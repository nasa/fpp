...
inside a <<Defining-State-Machines,state machine definition>>,
...
|`instance`
|A <<Defining-Component-Instances,component instance definition>> or
<<Defining-Topologies,topology definition>>
...
|`system`
|A <<Defining-Systems,system definition>>
...
You can also write a location specifier using the <<Defining-Modules,leading 
...
In this case, the name `.a` is resolve to `a`, not `M.a`.
The leading dot says to start the resolution at the top level,
regardless of where the location specifier appears in the model.
...
Notice that the tool uses the <<Defining-Modules,leading dot>>
