...
initialization format. In most cases, you should not provide code
for this phase. Instead, you should use the standard initialization format.
...
|`deinit`
|_T_ `TopologyAc.cpp`
| Deinitializtion code for an instance _I_ that has a non-standard
deinitialization format. In most cases, you should not provide code for
this phase. Instead, you should use the standard deinitialization format.
|In the file-private function `deinit`.
|The standard call to `deinit`.
...
In theory you should never have to write code for `instances`, `initComponents`,
or `deinit` -- this code can be be standardized --
