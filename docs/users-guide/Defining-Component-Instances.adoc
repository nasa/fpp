...
|`deinit`
|_T_ `TopologyAc.cpp`
|Code deinitializing _I_.
|In the file-private function `deinit`.
|The standard call to `deinit` if _I_ is an
active or queued component; otherwise none.
...
Code for `deinit` is required only if the user-written implementation of
a component instance manages its own F Prime queue.
If you use a standard F Prime active or queued component, then the framework
manages the queue, and this code is generated automatically.
