...
We will use the letter _I_ to refer generically to the name of a component
instance.

*Phases:*
Table <<init-specifier-phases>> shows the setup and teardown phases 
...
|`regCommands`
|_T_ `TopologyAc.cpp`
|Code for registering commands of _I_ with the command dispatcher.
...
|`readParameters`
|_T_ `TopologyAc.cpp`
|The method call for reading parameters from a file.
