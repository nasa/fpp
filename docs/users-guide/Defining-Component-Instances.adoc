...
needs one line of special code for reading its parameters.

You typically have to write code for `startTasks`, `stopTasks`,
and `freeThreads` only if you use a component that manages its
own F Prime task in a non-standard way.
If you use a standard F Prime active component, then this
code is generated automatically.

Code for `tearDownComponents` is required only if a component
needs to deallocate memory or release resources on program exit.
