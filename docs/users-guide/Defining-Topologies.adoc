...
highest level of software architecture in a
*deployment*, or executable binary program.
...
An F Prime deployment consists of a topology _T_;
...
Next we define a *deployment topology*.
This is the topology associated with an F Prime deployment.
To do this, we write the keywords `deployment topology`.
Then we write the name of the topology and
...
In this case the imported topology is called a *subtopology*
of the topology into which it is imported.
In this section of the User's Guide, we explain how to define and
use subtopologies.

==== Importing Instances and Connections

To import a topology `A` into a topology `B`, you do the following:

. Define the topology `A` as described in the previous sections,
but without the keyword `deployment` at the start of the definition.
Omitting the keyword `deployment` tells the analyzer that the
topology definition `A` is for import, and not for code generation.

. Inside topology `B`, write `import A`.
...
. Define a topology `A` that is for import only and
is not used in a deployment.

. Define a deployment topology `B` that imports `A`.

You may add instances and connections as usual to both
`A` and `B`, as shown by the dots.
...
In this case, `A` may or may not be a deployment
topology; neither `B` nor `C` may be a deployment
topology.
...
==== Using an Imported Topology as a Deployment

Sometimes it is useful to import a topology `B` into a
topology `A` and also to use `B` as a deployment topology.
For example, `A` may be a system topology `System`, `B` may be
a subsystem topology `Subsystem`, and it may be useful to create
a subsystem deployment for testing the subsystem.
In this case you can do the following:

. Define a non-deployment topology `Subsystem`

. Define a deployment topology `System` that imports `Subsystem`
and contains other instances and connections.

. Define a deployment topology `SubsystemDeployment` that just imports
`Subsystem`.
...
----
topology Subsystem {
  # Subsystem instances and connections
}

deployment topology System {
  import Subsystem
  # System instances and connections
}

deployment topology SubsystemDeployment {
  import Subsystem 
}
----

Now you can use `Subsystem` as an imported subsystem and `SubsystemDeployment`
as a subsystem deployment.
...
The concept of a port interface instance is most useful
in connection with module templates, which we will describe
<<Defining-and-Using-Module-Templates,in a later section>>.
When expanding module templates, it will be useful to treat component
...
When defining a deployment topology, you may (but are not required to)
...
To group the channels of a deployment topology _T_ into packets,
you write a
...
Note that telemetry packet sets are permitted only inside
deployment topologies.
If you define a topology without the `import` keyword
and the definition includes a telemetry packet set,
