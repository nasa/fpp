...
one topology into another one and to encapsulate topologies
into black boxes using topology *ports*.
...
`instance A` inside topology `B`, like this:
...
Notice that the the topology `A` is not distinguished from a component instance
`A`. This is because both are considered interface instances. Typically
topologies are named in `PascalCase` while component are named in `camelCase`.
Interface instances refer to either component instances or topologies. They are FPP
constructs that include port _instances_ and therefore can be thought of as the
instantiation of a port _interface_.

[NOTE] 
.Backwards compatibility
==== 
`import A` may be used in place of `instance A`. The `import` keyword was
kept for backwards compatibility and is 1:1 compatible with `instance`.
====
...
topology A {
  ...
  instance B
  instance B # Error: B imported twice
...
A topology port is a port name assigned to the topology that maps
to another port instance. It is best to think of a topology port
like a port alias for a connection endpoint _inside_ the topology.

The concrete behavior of a port will be defined by the port instance
it maps to. If that port instance is another topology port, this behavior
will be recursively resolved to a port instance of a component instance
definition.
...
topology A {
  ...
  port bOut = b.somePortOut

}

topology B {

  instance A # import the topology
  instance c # import another instance
...
Because the topology port `A.bOut` is really just an alias to
`b.somePortOut`, this topology will eventually resolve to:
...
It can be useful to use topology ports in most cases when a subtopology
is imported into a parent topology. Defining topology ports allows
encapsulation of port connections and signals to a developer importing
a topology which port instances need to interface with the outer topology.

===== Implementing interfaces

A topology definition may refer to a port interface through an
`implements` clause which will verify that the set of port instances
defined in the port interface exist in the topology definition.

The usage of the `implements`` clause is defined in <<Defining-and-Using-Port-Interfaces_Using-Port-Interfaces-in-Topology-Definitions,
a separate section>>.

We recommend using the implements clause whenever a topology definition
needs to conform to a port interface. This ensures compatibility
expectations with the topology when imported into another larger
topologies.
