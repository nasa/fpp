...
In this example there are two connection graphs and two connections:

. A connection from `c1.pOut` to `c2.pIn` in connection graph `C1`.

. A connection from `c2.pOut` to `c1.pIn` in connection graph `C2`.
