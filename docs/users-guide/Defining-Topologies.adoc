...
each port instance is actually an array of one or more port instances.
When the size of the array exceeds one, you
have to specify the port number (i.e., the array index)
...
For example, the `RateGroups` graph of the reference topology in the F Prime 
...
*Automatic matching:*
After resolving the explicit port numbering, the FPP translator applies
*matched numbering*.
...
If the matching does not follow this pattern than automatic matched
...
Since the allocation and deallocation connections involve different
