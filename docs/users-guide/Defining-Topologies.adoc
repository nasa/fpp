...
* All the component instances named in `B` are visible in `A`.

* Connections that span `A` and `B` go directly between
  component instances named in `A` and component instances named in `B`.
...
connections _to and from topologies_.
...
In this way, you can think of imported subtopologies as
...
are compatible.
Here `B.p` is an input port if and only if
`b.p` the actual port to which the topology port `B.p` is mapped,
is an input port.
...
of `A` and `B`:
the connection goes from an instance of `A` to the interface
...
in the interface of _T_ to a port of a component instance of an imported 
topology of _T_.
However, we can use one or more interfaces to specify and 
check that the interface of _T_ contains all the ports that it should.
To do this, we write the keyword `implements` followed by list of names
that refer to interfaces.
The FPP analyzer will then check that the topology provides
all the ports required by each of the interfaces.
...
* We delete line `port p = a.p` is deleted from the model.
...
Try these examples in `fpp-check`, to make sure you understand what is 
...
@ Topology T provides the ports of interfaces I and J
...
containing implements clauses will also be a correct model if
they are deleted.
However, they are useful for documenting and checking intended behavior.
For example, if the intent of a topology _T_ is to provide a certain
interface, and there is no `implements` clause documenting the intent,
then a developer could inadvertently change the contract of _T_ by removing or
modifying a port in its interface.
If _T_ has an implements clause, then the change will cause an error,
...
different sections of of FPP code (for example, topologies) that are largely 
the same, but that differ in particular ways that you specify.
...
that can participate in connections, as in the connection shown.

The concept of a port interface instance is new as of v3.2.0 of FPP.
In previous versions, the only way to import a topology was to
