...
The channel name must name a channel specified in the component
that is the type of an instance of the topology.
...
otherwise one more than the identifier of the previous event.
...
For example, in the model shown above, if we omit the definition
of packet `P2`, then an error will result, because neither channel
