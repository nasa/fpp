...
For asynchronous ports, arguments are always passed by value. For synchronous ports, primitive arguments pass by value and non-primitive types pass by const reference. The passing by const reference is an optimizaton to avoid copying large values between function calls. Primitive types are types that hold <<Defining-Constants_Expressions_Primitive-Values,primitive values>>. This pass by logic can be modified with reference parameters.
...
port P3(t: T) # t is passed by value for asynchronous invocations and by const reference for synchronous invocations
----
