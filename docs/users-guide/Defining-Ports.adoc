...
When a parameter is not marked `ref` (the default case),
we say that it is a *value parameter*.
...
. Using a value parameter in a synchronous invocation.

. Using a value parameter or a reference parameter in an asynchronous 
invocation.
...
auto-generated code.
If you also need to use `P` in a synchronous context, and you need
pass-by-value semantics in that context, then you can copy the
...
in a synchronous context (the intended use of `ref`); or (2)
the parameter carries large data and you intend to use it in an asynchronous
context (the result of an autocoder quirk, which will be fixed in
