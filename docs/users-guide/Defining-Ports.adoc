...
FPP uses the following logic when passing arguments through ports.

[width="15%"]
|=======
| |Synchronous port invocation |Asynchronous port invocation
|Primitive type |Pass by value |Pass by value
|Non-primitive type |Pass by const reference |Pass by value
|=======

For asynchronous ports, arguments are always passed by value. For synchronous ports, primitive arguments pass by value and non-primitive types pass by const reference. The passing by const reference is an optimization to avoid copying large values between function calls. Primitive types are types that hold <<Defining-Constants_Expressions_Primitive-Values,primitive values>>. This pass by logic can be modified with reference parameters.

The following example illustrates how parameters are passed.

[source,fpp]
----
type T
port P1(a: U32) # a is always passed by value
port P2(t: T) # t is passed by value for asynchronous invocations and by const reference for synchronous invocations
----
...
When a reference parameter is used in a synchronous port invocation, we say that the parameter has pass-by-reference semantics. However, when a reference parameter is used in an asynchronous port invocation, the parameter has pass-by-value semantics.
