...
Furthermore, when `ref` is not specified and we say the parameter pass-by type is unspecified and we opt for a default behavior. If the parameter is used in a synchronous port invocation, the parameter is passed by value and has *pass-by-value semantics* if it is a primitive type. If it is not a primitive type, it is passed by const reference.
...
[width="15%"]
|=======
|1 |Synchronous port invocation |Asynchronous port invocation
