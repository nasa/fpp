...
(2) then providing a reference to the data on the stack.
For example:
...
[source,cpp]
----
virtual void pIn_handler(const T& a, T& b);
----

Notice that parameter `a` is not marked `ref`, so it is
translated to `const T& a`, as discussed in the previous section.
On the other hand, parameter `b` is marked ref, so it
is translated to `T& a`.
