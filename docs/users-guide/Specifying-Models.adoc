...
Because constant and type definitions are short, include specifiers are less
useful for writing these definitions.
However, we describe include specifiers here for completeness, anticipating
that they will become useful in later versions of FPP.
...
In any case, the source text in an included file must consist of complete syntactic
...
% echo "a = 0" > a.fpp
% fpp-check
module M { constant include "a.fpp" }
constant b = M.a
^D
fpp-check
stdin: 1.21
...
Whenever a model is split into two or more files, one file _f_ may use symbols
...
TODO: When including a file that defines symbols, the location of
each symbol is the location of the file not the location where the file is 
included.
