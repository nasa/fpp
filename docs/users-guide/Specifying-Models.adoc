...
We focus on constants and types, because those elements of FPP
...
For example, you may run step 1 to locate all the symbols exported by one or 
more modules,
and then run step 2 several times to develop files that depend on those 
modules,
without running step 1 again.
...
For example, a use of constant `M.a` in an expression inside module `M`
may be spelled either `a` or `M.a`.
When referring to `M.a` in a locate specifier in module `M`, you must
write `a` and not `M.a`.
The reason for this restriction is to facilitate dependency analysis,
