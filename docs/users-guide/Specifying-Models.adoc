...
* We focus on defining constants and types, because those 
...
definition that appear in other files.
...
for all the definitions _that could be used_ in _F_.
...
For the current version of FPP, the kind of definition can be `constant` or `type`.
...
However, by reporting dependencies at the granularity of individual definition, 
