...
. Create files `a.fpp` and `b.fpp` as described
above.
...
units that could appear at the point where the include specifier appears.
...
Suppose `b.fpp` contains the definition `constant b = 1`.
...
For example, if your included FPP files all end with the suffix
`.included.fpp`, then you can run the following command to
find non-included FPP files at or below the current directory:

----
find . -name '*.fpp' | grep -v '\.included\.fpp$'
----

Notice that `find` takes a file glob and `grep` takes
a regular expression as input.
The `-v` (reverse) option to `grep` excludes files matching
the pattern.
The `$` symbol in the pattern matches the end of the file name.
