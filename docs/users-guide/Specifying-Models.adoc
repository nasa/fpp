...
When you include a file that defines in another file, the location of
...
and (2) a superset of the location specifiers for the symbols used in that 
code.
...
as described in the previous section, but then we remove `b.fpp`.
...
a warning or error, or to identify files that need to be generated.
To record missing dependencies, use the `-m` option.
It takes as an argument the name of a file, and it writes missing dependencies (if any)
to that file.
...
==== Dependencies on Included Files
...
declarations that you have to construct by hand when writing F Prime XML.
...
Doing this provide as a kind of "import statement," if that is desired, e.g.,
