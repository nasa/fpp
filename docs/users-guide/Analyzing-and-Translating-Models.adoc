...
for `FppConstantsAc.hpp` is _prefix_ `pass:[_]FppConstantsAc_HPP`,
where _prefix_ is derived from the absolute path of the current
...
the prefix is `pass:[_]home_user`.

The `-p` option, if present, is applied to the include guard
prefix.
