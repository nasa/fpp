...
as the types arguments or as the return type.
Again the enumerated constants are specified inline.

* The F Prime XML format for commands and for events can
contain enumerations as the types of arguments.
...
Therefore, when translating an F Prime XML file file that 
