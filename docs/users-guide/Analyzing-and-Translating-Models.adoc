...
<<Defining-Types_Abstract-Type-Definitions,Abstract type definitions>>
represent {cpp} classes that you write directly in {cpp}.
When you use an abstract type _T_ in another FPP definition _D_ (for example, 
an array definition)
and _D_ is translated to XML, then the use of _T_
becomes an `include_header` node in the generated XML
for _D_.
...
to a {cpp} class, the class will expect a header file `T.hpp`
to exist in the current directory. It is up to you to provide that header.
