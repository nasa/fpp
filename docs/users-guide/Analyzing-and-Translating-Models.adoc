...
A *ground dictionary* is a specification of all the commands,
...
commands, events, telemetry, and data products; and decoding of
data products.
...
*Tool behavior:*
For each topology _T_ defined in the input files, `fpp-to-dict` writes a file
...
The most common use case is to run `fpp-to-dict` with a single topology _T_
in the input files.
_T_ describes all the component instances and connections in a FSW
binary application, and the generated dictionary _T_ `TopologyDictionary.json`
is the dictionary for the application.
In this case, if _T_ imports subtopologies, then you use `-i` to
import the source files defining those topologies.
