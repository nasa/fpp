...
The FPP tool suite provides a limited capability to translate F Prime
XML files to FPP.
...
Currently only translation for types and enumerations is provided.
However, you can translate your types to FPP now, and integrate
the FPP types with the rest of your model, which is in XML.
Later, when more of FPP becomes available, you can translate more
of the model.

The purpose of the XML-to-FPP translation is to do most of the work in 
translating an existing XML
model into FPP.
...
The reason is that the XML parser used by the tool requires
named files.
...
Notice that the translation from XML has added explicit default
values.
The reason for this is explained below.

*Default values:*
There are two issues to note in connection with the translation of default 
values.

First, in FPP you can omit the default value for a definition;
if you do that, the automatic default is used.
By contrast, in F Prime XML, you _must_ supply default values for array
elements (but you _may not_ supply default values for struct members
...
representation, then `fpp-from-xml` translates them explicitly,
even if (as in the example above)
they are the automatic values and so could be implicit.

Second, to keep the translation simple, only numeric values,
string values, and Boolean values are translated.
Array, struct, enum, and abstract type values are not translated, because the
types cannot be easily inferred from the form of those values.
In this case, the translator adds an annotation
...
Only XML arrays have default values, enums and structs
do not. (Again, this is a limitation of the F Prime XML representation.)
So these issues pertain only to array translation.

*Complex serializable members:*
The F Prime Serializable XML type can
contain arrays and enumerations as member types.
The array sizes and enumerated values are specified inline, as
part of the member definition.

FPP does not represent these inline type definitions.
In FPP, type definitions are always explicit and named,
so they can be reused.
Therefore, when translating a Serializable XML type that contains
an array or enumeration member, `fpp-to-xml` does the following: (1) translate
the type to an FPP type; and (2) translate the member using that type.
...
. The namespace `N` is translated to module `N`.
. The Serializable type `N::S` is translated to struct `N.S`.
...
The handling of inline enumeration types is similar.

While the FPP translator will translate inline
array and enum members in the manner described, it may be a
better development strategy not to use this capability.
Instead, you can first refactor your FSW
to eliminate these inline members, and then translate from
XML to FPP.
This order of operations presents less risk of breaking
your XML model, and all the code depending on it, at the
point of translating to FPP.

In any event, the use of inline array and enum members is being phased
out in favor of named array and enum types as we
migrate to FPP.
