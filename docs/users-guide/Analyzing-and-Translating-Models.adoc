...
FPP can translate the connection graphs in topology _T_ to text, which can then be read 
by a companion tool called `fprime-layout`.
`fprime-layout` reads a topology connection graph text file and produces a
...
*Tool name:* The tool for generating layout text files is called
`fpp-to-layout`.
...
_T_ `Layout` and writes layout text files for each of the connection graphs in _T_.
