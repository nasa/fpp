...
*Inline array and enum definitions:*
The following F Prime XML formats may include inline
definitions of arrays and enumerations:

* In the Serializable XML format,
arrays and enumerations may appear as member types.
...
In each case, the array sizes and enumerated constants are specified 
inline, as
part of the definition of the member, argument, return type, etc.

FPP does not represent these inline array and enum definitions directly.
In FPP, array and enum definitions are always named, so they can be reused.
Therefore, when translating an F Prime XML file that 
contains inline definitions, `fpp-to-xml` does the following: (1) translate
each inline definition to a named FPP array or enum; and (2) use the 
corresponding named types in the translated FPP struct or port.
...
. The tool translates Serializable type `N::S1` to 
struct type `N.S1` (that is, struct type `S1` defined
in module `N`).
...
The tool handles inline arrays similarly, except that the array
type has no name in the XML representation, so the translator
generates one.
For example, here is an F Prime Serializable XML type
`N::S2` containing a member `m` that is an array of 10 `U32`
values:

----
% cat > S2SerializableAi.xml
<serializable namespace="N" name="S2">
  <members>
...
----
% fpp-from-xml S2SerializableAi.xml
module N {
...
. The tool translates Serializable type `N::S2` to the struct type 
`N.S2`.

. The tool generates an array type `N.S2_m` to represent the
type of member `m` of struct `N.S2`.
The name `S2_m` is generated by the translator.

. The tool gives member `m` of struct `N.S2` the type `N.S2_m`.

If you wish to translate an XML model to FPP, and that model contains
inline arrays or enums, then we suggest the following procedure:
