...
in FPP (either in the input model itself or in a dependency file) and each type must be
...
For more information on defining types, see the 
<<Defining-Types_Alias-Type-Definitions,alias type
definition>> section.
