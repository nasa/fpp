...
`fpp-to-cpp` has two more options `-t` and `-u`.
These options are for generating component "templates" or
...
it generates all the code necessary for building and running unit tests.
This code consists of (1) the autocoded {cpp} for constants, ports,
types, components, and
topologies described in the previous sections; and (2)
support code for testing each component definition _C_ in the input.
...
`-t` _file_: Write the names of the generated component implementation
template files to _file_.
...
You can also specify these options together with the `-u` option.
In this case `fpp-depend` writes the names of autocoded
or template unit test files.
For example:
...
see the
https://github.com/fprime-community/fpp/wiki/Analysis-Data-Structure[FPP wiki].

*JSON format:*
To understand this subsection, you need to know a little
bit about case classes in Scala.
For a primer, see 
https://github.com/fprime-community/fpp/wiki/Pure-Functional-Programming-in-Scala#use-case-classes-for-pattern-matching[this wiki page].
