...
Currently translation for types and enumerations is available.
You can translate these elements in your XML model now,
and use FPP to represent them going forward.
To integrate the FPP types and enumerations with the rest of the XML model,
use the
...
FPP does not represent these inline type definitions directly.
In FPP, type definitions are always named, so they can be reused.
Therefore, when translating a Serializable XML type that contains
an array or enumeration member with an inline type definition, `fpp-to-xml` 
does the following: (1) translate
the inline definition to a named FPP type; and (2) use the named type in the 
translated member.
...
. The Serializable type `N::S` in the XML is translated to struct `N.S` in FPP.
...
While `fpp-from-xml` will translate inline
array and enum definitions in the manner described, it may be
better not to rely on this capability.
Instead, you can first refactor your XML model and your FSW
to eliminate all inline type definitions in the XML, and then translate from
XML to FPP.
This approach has better separation of concerns, because you can refactor,
test the refactoring, translate, and test the translation.
If you translate first, then you will still need to refactor; but then you must 
implement and test the translation and the refactoring at the same time.
