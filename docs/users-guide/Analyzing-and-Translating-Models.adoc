...
|Array _A_ outside any state machine or component|_A_ `ArrayAc.{hpp,cpp}`
|Array _A_ in state machine _M_|_M_ `_` _A_ `ArrayAc.{hpp,cpp}`
|Array _A_ in component _C_|_C_ `_` _A_ `ArrayAc.{hpp,cpp}`
|Enum _E_ outside any state machine or component|_E_ `EnumAc.{hpp,cpp}`
|Enum _E_ in state machine _M_|_M_ `_` _E_ `EnumAc.{hpp,cpp}`
...
|Struct _S_ outside any state machine or component|_S_ `SerializableAc.{hpp,cpp}`
|Struct _S_ in state machine _M_|_M_ `_` _S_ `SerializableAc.{hpp,cpp}`
|Struct _S_ in component _C_|_C_ `_` _S_ `SerializableAc.{hpp,cpp}`
|Alias type _A_ outside any state machine or component|_A_ `AliasAc.{hpp,h,cpp}`
|Alias type _A_ in state machine _M_|_M_ `_` _A_ `AliasAc.{hpp,h,cpp}`
...
|Deployment topology _T_|_T_ `TopologyAc.{hpp,cpp}`
|Telemetry packet set _S_ in topology _T_|_T_ `_` _S_ `TlmPacketsAc.{hpp,cpp}`
|===
...
Outside of any state machine or component definition,
...
and `M_AArray.cpp`.
In any case, the {cpp} namespace is given by the enclosing
FPP modules, if any.
...
`fpp-to-cpp` also extracts the definitions of <<Defining-Topologies,deployment 
topologies>> from the source files.
For each deployment topology _T_ defined in the source files, `fpp-to-cpp`
...
You can also examine the files `RefTopologyAc.hpp` and `RefTopologyAc.cpp`
in build cache for the `Ref` example in the F Prime repository.

*Telemetry packets:*
If a deployment topology _T_ defines a
<<Defining-Topologies_Telemetry-Packets_Telemetry-Packet-Sets,
telemetry packet set _S_>>, then `fpp-to-cpp` generates files
_S_ `_` _T_ `TlmPacketsAc.hpp` and _S_ `_` _T_ `TlmPacketsAc.cpp`
These files provide the code needed to export the packet
definitions to the F Prime Telemetry Packetizer component.
...
=== Rewriting FPP Source

TODO: Give the new name and explain the new purpose of the tool:
not formatting, but rewriting with expansion.
...
TODO: Explain the syntax expansion option.
Use `-e` as the switch for syntactic expansion.
This will add state machine enums and expand templates.
...
. Identify one or more files _F_ that you want to translate.
. <<Specifying-Models-as-Files_Computing-Dependencies,Compute the dependencies>> _D_ of _F_.
. Run `fpp-to-dict` _D_ _F_.

Except for the tool name, this procedure is identical to the one given for
<<Analyzing-and-Translating-Models_Checking-Models,checking models>>.
...
*Tool behavior:*
For each <<Defining-Systems,system>> _S_ defined in the
input files, `fpp-to-dict` writes a file _S_ `SystemDictionary.json`.
Here _S_ is the qualified name of the system, with dots
replaced by underscores.
For example, for a system `S` defined in module `M`,
the file name is `M_SSystemDictionary.json`.
The generated dictionary records all the definitions in the
single deployment topology referred to by _S_.
...
For each <<Defining-Topologies,deployment topology>> _T_
defined in the input files, `fpp-to-dict` 
also writes a file _T_ `TopologyDictionary.json`.
Here _T_ is the simple name of the topology, without qualifiers.
This behavior is for backwards compatibility with the F Prime tools.
In the future, we plan to revise to tools to use the system dictionary;
when we do that, we will remove the code generation for the
topology dictionary.

Here is a common use case:

* The files _F_ define a single system _S_ and a single
deployment topology _T_.  _S_ refers to _T_.
_T_ describes all the component instances and connections in a FSW
application, and the generated dictionary _S_ `SystemDictionary.json`
is the dictionary for the system.

* If _T_ imports subtopologies, then those subtopologies are defined
in the dependency files _D_.
The subtopologies are part of the model, but no dictionaries
are generated for them.

*F Prime configuration:*
When you run `fpp-to-dict` on an FPP model, the model must provide the
<<Defining-Constants_Framework-Constants,framework constants>>
and
<<Defining-Types_Framework-Types,framework types>>
specified in the
https://fprime.jpl.nasa.gov/latest/docs/reference/fpp-json-dict/[F Prime
...
* The `-d` option works in the same way as for
<<Analyzing-and-Translating-Models_Generating-C-Plus-Plus_Constant-Definitions,
`fpp-to-cpp`>>.
...
* `-f`: Format the JSON output with whitespace. Without this option `fpp-to-json`
will write JSON with no spaces or newlines. With this option, nested JSON
objects will be indented.

=== Translating Older XML to FPP
