...
is translated to the file `AArrayAi.xml` when it appears outside
any component definition.
Inside the definition of component `C`, it is translated to
the file `C_AArrayAi.xml`.
...
* In the Prime Serializable XML format,
...
. The tool translates Serializable type `N::S1` in XML to 
struct type `N.S1` in FPP.

. The translator generates an enum type `N.E` to represent the
type of member `m` of struct `N.S1`.

. The translator assigns member `m` of struct `N.S1` the type `N.E`.

Inline arrays are handled similarly, except that the array
