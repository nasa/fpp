...
*Tool name:* The tool for generating layout txt files is called
`fpp-to-layout`.
...
*Tool behavior:*
For each topology _T_ defined in the input files _F_, `fpp-to-layout` creates a directory for 
_T_ `TLayout` and writes layout `txt` files for each of the connection graphs in _T_.

The layout file format is specified in the
https://github.com/fprime-community/fprime-layout/wiki/Topology-Input[FPL Input Format Wiki].

*Options:*
`fpp-to-layout` provides the following options:
...
`fpp-to-layout` generates text files that can be read by a companion tool called
`fprime-layout`.
`fprime-layout` reads topology connection graph text files and produces a
