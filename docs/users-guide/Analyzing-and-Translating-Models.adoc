...
The tool `fpp-to-fpp` accepts FPP source files as input
and rewrites them after applying transformations.
You can use this tool to expand the FPP model.
...
By default, `fpp-to-fpp` does not resolve include specifiers.
For example:
...
The `-i` option causes `fpp-to-fpp` to resolve include specifiers.
For example:
