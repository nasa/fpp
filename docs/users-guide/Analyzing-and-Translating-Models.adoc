...
These files define a class _C_ `TesterBase` for unit testing _C_.
This class contains helper code, for example an input port and history corresponding
to each output port of _C_.
...
These files partially define a class _C_ `Tester` that is derived from `GTestBase`.
You can fill in the partial implementation to provide unit tests for _C_.
If you are not using _C_ `GTestBase`, then you can modify
...
These options write the generated autocoded file names or the generated
